...
class SwinDualBranch(nn.Module):
    ...
    def forward(self, x, y: Optional[torch.Tensor] = None, return_attn: bool = False):
        ...
        if self.use_wsdan:
            # ✅✅ 强制修复: 确保 feat 是 [B, C, H, W] 格式再传给 WSDAN
            if feat.dim() == 4:
                if feat.shape[1] != self.backbone.num_features: # 如果第二维不是通道数
                    # 尝试转换: 假设是 [B, H, W, C]
                    if feat.shape[3] == self.backbone.num_features:
                        feat = feat.permute(0, 3, 1, 2) # (B, H, W, C) -> (B, C, H, W)
                    else:
                        # 如果形状依然不对，抛出明确错误
                        raise RuntimeError(f"Feature map shape {feat.shape} is not [B, C, H, W] or [B, H, W, C]. "
                                         f"Expected channel dim (C) to be {self.backbone.num_features}.")
            else:
                raise RuntimeError(f"Feature map must be 4D, got {feat.dim()}D with shape {feat.shape}")
