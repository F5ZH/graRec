...
def crop_and_resize_by_attn(images: torch.Tensor, attn_maps: torch.Tensor, output_size: int = 224):
    ...
    attn_max, _ = torch.max(attn_maps, dim=1)
    flat_attn = attn_max.view(B, -1)
    max_idx = torch.argmax(flat_attn, dim=1)
    center_y = max_idx // W
    center_x = max_idx % W
    ...
    crops = []
    for b in range(B):
        crop = images[b, :, y0[b]:y1[b], x0[b]:x1[b]]
        crop_resized = F.interpolate(crop.unsqueeze(0), size=(output_size, output_size), mode='bilinear', align_corners=False)
        crops.append(crop_resized)
    return torch.cat(crops, dim=0)

# -------------------------
# Heterogeneous Dual Branch Model (Swin Global + ConvNeXt Local)
# -------------------------
class HeteroDualBranch(nn.Module):
    def __init__(self, swin_arch: str, convnext_arch: str, num_classes: int, use_wsdan: bool = True, K: int = 8,
        ...
        }.get(swin_arch, 'swin_base_patch4_window7_224')
        ...
        }.get(convnext_arch, 'convnext_base')

        self.backbone_global = timm.create_model(self.swin_name, pretrained=pretrained, num_classes=0, img_size=img_size)
        self.backbone_local = timm.create_model(self.convnext_name, pretrained=pretrained, num_classes=0)
        ...
        feat_dim_global = self.backbone_global.head.in_features if hasattr(self.backbone_global, 'head') else self.backbone_global.num_features
        feat_dim_local = self.backbone_local.head.in_features if hasattr(self.backbone_local, 'head') else self.backbone_local.num_features

        self.global_pool = nn.AdaptiveAvgPool2d(1)

        if use_wsdan:
            self.wsdan = WSDAN(in_ch=feat_dim_global, K=K)

        # Global Branch (Swin)
        self.dropout_global = nn.Dropout(0.1)
        self.feat_proj_global = nn.Linear(feat_dim_global, 1024)
        if arcface:
            self.margin_head_global = ArcMarginProduct(1024, num_classes, s=s, m=m)
        else:
            self.fc_global = nn.Linear(1024, num_classes)

        # Local Branch (ConvNeXt)
        self.dropout_local = nn.Dropout(0.1)
        self.feat_proj_local = nn.Linear(feat_dim_local, 1024)
        if arcface:
    ...
    def get_attn(self, x: torch.Tensor):
        feat = self.backbone_global.forward_features(x)
        if feat.dim() == 4 and feat.shape[3] == feat.shape[1]:
    ...
    def forward(self, x, y: Optional[torch.Tensor] = None, return_attn: bool = False):
        # Global Branch Feature Extraction (Swin)
        feat_global = self.backbone_global.forward_features(x)
        if feat_global.dim() == 3:
            hw = int(math.sqrt(feat_global.shape[1]))
            feat_global = feat_global.view(feat_global.shape[0], hw, hw, feat_global.shape[2])
            feat_global = feat_global.permute(0, 3, 1, 2)
        elif feat_global.dim() == 4 and feat_global.shape[3] != 1:
            feat_global = feat_global.permute(0, 3, 1, 2)

        attn_maps = self.wsdan(feat_global) if self.use_wsdan else None

        pooled_global = self.global_pool(feat_global).flatten(1)
        ...
        # Logits Fusion: Weighted Average
        logits_fused = (self.lambda_global * logits_global + self.lambda_local * logits_local) / (self.lambda_global + self.lambda_local)
...
def train(args, fold: Optional[int] = None):
    ...
    model = HeteroDualBranch(
        swin_arch=args.swin_arch,
        convnext_arch=args.convnext_arch,
    ...
    if hasattr(model.backbone_global, "set_grad_checkpointing") and args.grad_ckpt:
        model.backbone_global.set_grad_checkpointing(True)
    if hasattr(model.backbone_local, "set_grad_checkpointing") and args.grad_ckpt:
        model.backbone_local.set_grad_checkpointing(True)
    ...
    bb_global_ids = set(id(p) for p in model.backbone_global.parameters())
    bb_local_ids = set(id(p) for p in model.backbone_local.parameters())
    head_params = [p for p in model.parameters() if id(p) not in bb_global_ids and id(p) not in bb_local_ids]
    bb_global_params = [p for p in model.parameters() if id(p) in bb_global_ids]
    bb_local_params = [p for p in model.parameters() if id(p) in bb_local_ids]

    optim_groups = [
        {"params": head_params, "lr": args.lr, "weight_decay": args.weight_decay},
        {"params": bb_global_params, "lr": args.lr * args.backbone_lr_mult, "weight_decay": args.weight_decay},
        {"params": bb_local_params, "lr": args.lr * args.backbone_lr_mult, "weight_decay": args.weight_decay},
    ]
    ...
    if args.freeze_backbone_epochs > 0:
        ...
        for p in model.backbone_local.parameters():
            p.requires_grad = False
        print(f"[Init] Freeze backbones for {args.freeze_backbone_epochs} epoch(s).")
...
def predict(args):
    ...
    model = HeteroDualBranch(
        swin_arch=ck_args.get('swin_arch', args.swin_arch),
        convnext_arch=ck_args.get('convnext_arch', args.convnext_arch),
        num_classes=num_classes,
        ...
        img_size=ck_args.get('img_size', 384)
    )
...
def get_args():
    ...
    ap.add_argument('--swin_arch', type=str, default='swin_base', choices=['swin_tiny', 'swin_small', 'swin_base', 'swin_large'])
    ap.add_argument('--convnext_arch', type=str, default='convnext_base', choices=['convnext_tiny', 'convnext_small', 'convnext_base', 'convnext_large'])
    ...
    ap.add_argument('--elr_mem', type=int, default=0)
    ...
    ap.add_argument('--class_balanced', action='store_true')
    ...
    ap.add_argument('--lambda_global', type=float, default=1.0)
    ap.add_argument('--lambda_local', type=float, default=0.5)
    ap.add_argument('--local_crop_size', type=int, default=224)
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        if args.val_dir and os.path.isdir(args.val_dir):
            ...
            full_ds = ImageFolder(args.train_dir, loader=safe_pil_loader)
